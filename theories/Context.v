...
Section SubstInstance.
  ...
  Inductive atnd : list (term o) -> var -> (term o) -> Prop :=
  | Atnd0 Delta A A' :
      A' = A.[ren o (+1)] -> atnd (A :: Delta) 0 A'
  | AtndS Delta x A B B' :
      atnd Delta x B -> B' = B.[ren o (+1)] -> atnd (A :: Delta) (S x) B'.

  Lemma atn_mmap {f : term o -> term o} {Gamma x A A'} :
  ...
  Lemma mmap_atn {f : term o -> term o} {Gamma x A'} :
  ...
  (*   (forall x B, atn Gamma1 x B -> atn Gamma2 (zeta x) B.[ren o xi]) -> *)
  (*   atn (mmap (subst1 (ren o (+1))) Gamma1) x A -> *)
  (*   atn (mmap (subst1 (ren o (+1))) Gamma2) (zeta x) A.[ren o (0 .: xi >> (+1))]. *)
  (* Proof. *)
  (*   intros H1 H2. *)
  ...
  (*   (forall x C, atnd Delta1 x C -> atnd Delta2 (xi x) C.[ren o xi]) -> *)
  (*   atnd (A :: Delta1) x B -> *)
  (*   atnd (A.[ren o xi] :: Delta2) ((0 .: xi >> (+1)) x) B.[ren o (0 .: xi >> (+1))]. *)
  (* Proof. *)
  ...
  (*   atnd Gamma x A <-> *)
  (*   atnd (Delta ++ Gamma) (length Delta + x) A.[ren o (+(length Delta))]. *)
  (* Proof. *)
  (*   revert A x. *)
  ...
  (*   exists B, A = B.[ren o (+(length Delta))] /\ atnd Gamma x B. *)
  (* Proof. *)
  (*   revert A x. *)
  (*   induction Delta; intros. *)
  (*   - exists A. *)
  (*     simpl in H. *)
  (*     rewrite Plus.plus_0_r in H. intuition autosubst. *)
  (*   - asimpl. simpl in *. *)
  (*     rewrite plusnS in *. *)
  (*     inv H. *)
  ...
  (*   atnd Delta x A <-> atnd (B :: Delta) (S x) A.[ren o(+1)]. *)
  (* Proof. *)
  (*   apply atnd_steps with (Delta := B :: nil). *)
  (* Qed. *)

  (* Lemma atnd_repl Gamma A Delta : *)
  (*   (atnd (Delta ++ A :: Gamma) (length Delta) A.[ren o (+ S (length Delta))]) /\ *)
  (*   (forall x B A', *)
  (*       x <> length Delta -> atnd (Delta ++ A :: Gamma) x B -> *)
  (*       atnd (Delta ++ A' :: Gamma) x B). *)
  (* Proof. *)
  (*   split. *)
  (*   - pose proof (atnd_steps 0 (A :: Gamma) Delta A.[ren o (+1)]) as H. asimpl in H. *)
