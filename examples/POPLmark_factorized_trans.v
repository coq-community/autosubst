...
Lemma wf_weak xi Delta1 Delta2 A A' :
  wf_ty Delta1 A ->
  (forall x B, atnd Delta1  x B -> exists B', atnd Delta2 (xi x) B') ->
  A' = A.[ren xi] ->
  wf_ty Delta2 A'.
Proof.
  autorevert A. induction A; intros; subst; ainv; simpl; eauto.
...
Corollary wf_weak1 Delta A A' B :
...
Corollary sub_weak1 Delta A A' B B' C :
...
Corollary sub_wf_1 Delta A B : SUB Delta |- A <: B -> wf_ty Delta A.
Proof. apply sub_wf. Qed.

Corollary sub_wf_2 Delta A B : SUB Delta |- A <: B -> wf_ty Delta B.
Proof. apply sub_wf. Qed.

Lemma sub_narrow Delta1 A B :
     SUB Delta1 |- A <: B -> forall Delta2,
     (forall x C1, atnd Delta1 x C1 -> exists C2, atnd Delta2 x C2 /\ SUB Delta2 |- C2 <: C1 /\ (forall C0 Delta xi, SUB Delta |- C2.[ren xi] <: C1.[ren xi] -> SUB Delta |- C1.[ren xi] <: C0 -> SUB Delta |- C2.[ren xi] <: C0)) ->
     SUB Delta2 |- A <: B.
Proof.
  induction 1; intros.
  - constructor.
    (eapply wf_weak; eauto).
    + intros. edestruct H0; eauto. eexists. eapply H2.
    + now autosubst.
  - constructor.
    eapply wf_weak; eauto.
    + intros. edestruct H0; eauto. eexists. eapply H2.
    + now autosubst.
  - edestruct H1 as [C2 [H1_1 [H1_2 H1_3]]]. eauto.
    econstructor; eauto.
    replace C2 with C2.[ren id] by now autosubst. eapply H1_3; autosubst; eauto.
  - constructor; eauto.
  - constructor; eauto.
    + eapply wf_weak; eauto.
    ...
    + eapply IHsub2.
      ...
      * edestruct H2 as [C2 H2']; eauto.
        eexists. intuition.
        econstructor; eauto.
        eapply sub_weak1; eauto.
        autosubst in *.
        eauto.
Qed.

Lemma sub_trans B Delta A : SUB Delta |- A <: B -> forall C, SUB Delta |- B <: C -> SUB Delta |- A <: C.
Proof.
  autorevert B. sind B.
  intros Delta A H_AB. induction H_AB; intros C H_BC; ainv; eauto using sub.
  - inv H_BC.
        ...
        * eapply IH; eauto; try somega.
          ...
          intros ? ? H_atnd. {
            ...
            - intuition.
              ...
              + autosubst in *.
                eapply IH; eauto; try rewrite ren_size_inv; somega.
            - intuition.
              + econstructor; eauto.
