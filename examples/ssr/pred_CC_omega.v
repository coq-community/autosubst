...
Proof.
  ...
  apply: (@pstep_beta _ (s2.[up tau]) _ (t2.[tau])); asimpl; eauto using psstep_up.
Qed.
...
Proof.
  move=> sb. elim: sb C D => {A B}
    [A C D|n m leq C D conv sb|A B1 B2 sb1 ih C D conv sb2]; eauto using conv_sub1.
  - inv sb; eauto using sub1, sub1_conv.
    + apply: sub_sort. move: conv => /inj_sort eqn. subst.
      exact: leq_trans leq _.
    + exfalso. exact: conv_prod_sort (conv_sym conv).
  - inv sb2; eauto using sub1, sub1_conv.
    + exfalso. exact: conv_prod_sort conv.
    + move: conv => /inj_prod[conv1 conv2].
      move: (ih _ _ conv2 H) => {ih} sub. inv sub.
      eapply SubI. eapply sub1_prod; eauto. eapply conv_prod => //. exact: conv_prod.
Qed.
